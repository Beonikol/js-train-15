...
function generateRandomPassword(length) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    const random = characters.charAt(randomInd);
    password += random;
  }
  return password;
}
...
function findMinMax(numbers) {
  ...
  if (!Array.isArray(numbers)) {
    console.log("Помилка: це не масив");
    return null;
  }
...
function calculateHypotenuse(a, b) {
  ...
  if (typeof a !== "number" || typeof b !== "number") {
    console.log("Помилка: данні повинні бути числом");
    return null;
  }
...
function roundObjectValues(obj) {
  ...
  const roundedEnt = entries.map(([key, value]) => {
  ...
  const roundedObj = Object.fromEntries(roundedEnt);
...
function calculateVolumeCylinder(radius, height) {
  ...
  if (typeof radius !== "number" || typeof height !== "number") {
    console.log("Помилка: обидва параметри повинні бути числами");
    return null;
  }
...
function getFractionalPart(num) {
  ...
  const value = num - Math.trunc(num);
...
function compareAndRound(num1, num2) {
  ...
  const value = Math.max(num1, num2);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  if (!Array.isArray(products)) {
    console.log("Помилка: обьєкт має бути масивом");
    return null;
  }

  if (typeof maxPrice !== "number") {
    console.log("Помилка: обьєкт має бути числом");
    return null;
  }
  ...
  const totalP = Math.fround(totalPrice);
  const maxP = Math.fround(maxPrice);
